...
def predict():
    try:
        ...
        input_data = pd.DataFrame([[drug_encoded, dosage, age]], columns=["Drug", "Dosage", "Age"])
        pred_encoded = model.predict(input_data)[0]
        side_effect = le_side.inverse_transform([pred_encoded])[0]
        proba = model.predict_proba(input_data)[0]
        confidence = round(max(proba) * 100, 1)
        return jsonify({"side_effect": side_effect, "confidence": confidence, "drug": drug_name, "dosage": dosage, "age": age})
